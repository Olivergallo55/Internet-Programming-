...
public class XMLSender {
	...
	// https://stackoverflow.com/questions/21001305/build-an-xml-with-jdom2-and-add-data
	// with modification
	// This document is a Tree with the elements as child nodes, the message element
	// is the root node
	...
	private static void readName(Element read) {
		GUI.area.append('<' + read.getChild("id").getChildText("name") + '>' + " " + "(<" + read.getChild("id").getChildText("email") + ">)" + " ");
	}
	...
	//insperation taken from https://howtodoinjava.com/java/xml/jdom2-read-parse-xml-examples/ but impklemented with modification 
	public void reciveXML(String document) {
		Document doc = null;
		SAXBuilder b = new SAXBuilder();
		try {
			doc = b.build(new StringReader(document)); // this is where i get the error
			printXML("\n###########################_INPUT_###########################", doc);
